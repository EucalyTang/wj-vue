...
export default {
  ...
  methods: {
    loadMembers() {
      var _this = this
      this.$axios.get('/member/' + this.pageSize + '/1').then((resp) => {
    ...
    handleCurrentChange(page) {
      var _this = this
      this.$axios.get('/member/' + this.pageSize + '/' + page).then((resp) => {
        if (resp && resp.status === 200) {
          _this.members = resp.data.content
          _this.total = resp.data.totalElements
        }
    ...
    deleteMember(member) {
      if (member.enabled == 1) {
        ...
        this.$confirm('此操作将永久删除该客户，是否继续？', '提示', {
          ...
          .then(() => {
            this.$axios.delete('/member/' + id).then((resp) => {
              if (resp && resp.status === 200) {
                this.loadMembers()
              }
            })
