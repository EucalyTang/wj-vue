<template>
  <div>
    <el-dialog title="修改子弹信息" :visible.sync="dialogFormVisible">
      <el-form
        ...
        <el-row>
          <el-col :span="20">
            <el-form-item label="级别" prop="level">
              <el-select v-model="selectedItem.level" placeholder="级别">
    ...
    <el-row style="margin: 18px 0px 0px 18px ">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        ...
        <el-breadcrumb-item>子弹管理</el-breadcrumb-item>
      </el-breadcrumb>
    </el-row>
    <el-card style="margin: 18px 2%;width: 95%">
      <bullet-add @onSubmit="onSubmit"></bullet-add>
      <el-table
        :data="items"
        stripe
        style="width: 100%"
      ...
      >
        ...
        <el-table-column prop="quantity" label="数量/盒" fit> </el-table-column>
        <el-table-column prop="level" label="等级" fit> </el-table-column>
...
<script>
import BulletAdd from './BulletAdd'
export default {
  name: 'Bullet',
  components: {
    BulletAdd,
  ...
  methods: {
    loadItems() {
      var _this = this
      this.$axios
        .get('/admin/item/bullet/' + this.pageSize + '/1')
        .then((resp) => {
    ...
    handleCurrentChange(page) {
      var _this = this
      this.$axios
        .get('/admin/item/bullet/' + this.pageSize + '/' + page)
        .then((resp) => {
          if (resp && resp.status === 200) {
            _this.items = resp.data.content
            _this.total = resp.data.totalElements
    ...
    deleteItem(item) {
      this.$confirm('此操作将永久删除该商品，是否继续？', '提示', {
        ...
        .then(() => {
          this.$axios.delete('/admin/item/bullet/' + id).then((resp) => {
            if (resp && resp.status === 200) {
              this.loadItems()
            }
          })
